...
###############################################################################
print('\nwordfeature.py program started ',
...
fwh = pd.read_csv(
    fullfilename, nrows=prm_wf['max_htmltagwordfeatures'], header=0)
...
def get_features(iurl):
    ...
    if soup != None:  # if file load was successful
        current_features = cm.featuremake(
            soup, fwt, fwl, fwh, current_url, cnames, prm_wc, prm_wf, nlp)
...
if 'label' in urls.columns:
    ...
    df = pd.concat([pd.DataFrame(url_list), pd.DataFrame(
        label_list), pd.DataFrame(features_list)], axis=1)
else:
    # assert len(cnames) == len(features_list[0]) + 1
    df = pd.concat(
        [pd.DataFrame(url_list), pd.DataFrame(features_list)], axis=1)

df.columns = cnames

fullfilename = prm_wf['wordfeature_output_fname']
df.to_csv(fullfilename, index=False, sep="|")
print('\n\nwrote csv word feature file ',
      prm_wf['wordfeature_output_fname'], ' with shape=', df.shape)
