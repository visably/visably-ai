...
def geturl(current_fname):
    ...
    try:
        ...
        response = s3.get_object(Bucket="ml-scraper",
                                 Key="031ccc_com.html")
        html_code = response['Body'].read().decode('utf-8')
...
def gettextwords(soup, prm, nlp=None):
    ...
    single_words = [w for sublist in single_words for w in sublist]
    ...
    char_to_strip = list(range(32, 64))+list(range(91, 97)) + \
        list(range(123, 128))  # with digits to be deleted
    ...
    if prm['verbose']:
        print('   Dropping ', before_count - len(single_words),
              ' text words with no alpha characters.')
    # Drop small and large words
    before_count = len(single_words)
    single_words = [w for w in single_words if len(
        str(w)) >= prm['min_word_len']]
    if prm['verbose']:
        print('   Dropping ', before_count - len(single_words),
              ' short text words with length less than', prm['min_word_len'])
    before_count = len(single_words)
    single_words = [w for w in single_words if len(
        str(w)) <= prm['max_word_len']]
    if prm['verbose']:
        print('   Dropping ', before_count - len(single_words),
              ' long text words with length greater than', prm['max_word_len'])
    # Drop stop words
    if prm['drop_stop_words']:
        ...
        if prm['verbose']:
            print('   Dropping ', before_count -
                  len(single_words), ' stop text words.')
    ...
    if nlp != None:
        ...
        single_words = [w.strip() for w in single_words if len(
            str(w).strip()) >= prm['min_word_len']]
        single_words = [w.strip() for w in single_words if len(
            str(w).strip()) <= prm['max_word_len']]
...
def getlinkwords(soup, prm):
    ...
    single_linkwords = [w for w in single_linkwords if len(
        str(w)) >= prm['min_word_len']]
...
def gethtmltagwords(soup, prm):
    ...
    single_htmltagwords = single_htmltagwords[:
                                              prm['max_document_html_tag_words']]
...
def featurenames(urls, fwt, fwl, fwh, prm):
    ...
    # Feature4 non features words contained in url count
    cnames.append('nfw_url_count')
    # Feature5 proportion of feature words to total words
    cnames.append('fwprop')
    ...
    if prm['word_frequency_features_prefix'] != 'none':
        cnames.extend([prm['word_frequency_features_prefix'] +
                       str(w).strip().replace("\n", "") for w in fwt.index])
    if prm['word_positions_stdev_features_prefix'] != 'none':
        cnames.extend([prm['word_positions_stdev_features_prefix'] +
                       str(w).strip().replace("\n", "") for w in fwt.index])
    cnames.append('ascii_total')  # total ascii character count
    if prm['ascii_features_prefix'] != 'none':
        # Only consider uppercase A-Z
        ascii_features = list(range(32, 97))+list(range(123, 128))
        temp = list(ascii_features)
        for i in range(len(temp)):
            temp[i] = prm['ascii_features_prefix']+str(temp[i])
        cnames.extend(temp)
    if prm['lword_frequency_features_prefix'] != 'none':
        # fraction of link words contained in text words)
        cnames.append('linkw_textw_overlap')
        cnames.extend([prm['lword_frequency_features_prefix'] +
                       str(w).strip().replace("\n", "") for w in fwl.index])
    if prm['hword_frequency_features_prefix'] != 'none':
        cnames.extend([prm['hword_frequency_features_prefix'] +
                       str(w).strip().replace("\n", "") for w in fwh.index])
    return cnames
...
def tfidffeaturenames(urls, word_vectorizer, lword_vectorizer, hword_vectorizer, prm):
    ...
    cnames.append('url_img_count')  # Feature8 count of image links in URL
    if prm['word_frequency_features_prefix'] != 'none':
        cnames.extend([prm['word_frequency_features_prefix']+str(w.strip())
    ...
    if prm['lword_frequency_features_prefix'] != 'none':
        # fraction of link words contained in text words)
        cnames.append('linkw_textw_overlap')
        cnames.extend([prm['lword_frequency_features_prefix']+str(w.strip())
                       for w in lword_vectorizer.get_feature_names()])
    if prm['hword_frequency_features_prefix'] != 'none':
        cnames.extend([prm['hword_frequency_features_prefix']+str(w.strip())
                       for w in hword_vectorizer.get_feature_names()])
    return cnames
...
def featuremake(soup, fwt, fwl, fwh, current_url, cnames, prm_wc, prm_wf, nlp=None):
    ...
    if len(all_textwords) > 0:
        ...
        f_textwords = [w.strip()
                       for w in all_textwords if w in list(set_f_textwords)]
        ...
        if prm_wc["verbose"]:
            print('   Total words=', tword_count, ', feature words=',
                  fword_count, ', unique feature words=', ufword_count)
        ...
        if prm_wf['word_frequency_features_prefix'] != 'none':
            print(
                len(fwt), prm_wf['word_frequency_features_prefix'], ';', end='')
        ...
        if prm_wf['word_positions_stdev_features_prefix'] != 'none':
            print(
                len(fwt), prm_wf['word_positions_stdev_features_prefix'], ';', end='')
            for feature in fwt.index:
                if fword_count > 0 and all_textwords.count(feature) > 0:
                    current_features.append(stdev(
                        [index for index, w in enumerate(all_textwords) if w == feature])/tword_count)
                else:
                    current_features.append(0)
        soup_text = soup.get_text().lower()

        # ascii_total total ascii character count
        current_features.append(len(soup_text))
        if prm_wf['ascii_features_prefix'] != 'none':
            # convert all lowercase a-z to A-Z uppercase
            ascii_codes = list(map(ord, str(soup_text).upper()))
            # determine ascii features to count from the cnames determined in featurenames
            ascii_features = [int(f.replace(prm_wf['ascii_features_prefix'], ''))
                              for f in cnames if f.find(prm_wf['ascii_features_prefix']) >= 0]
            print(len(ascii_features),
                  prm_wf['ascii_features_prefix'], ';', end='')
            for feature in ascii_features:
                current_features.append(
                    ascii_codes.count(feature)/len(ascii_codes))
        ...
        if prm_wf['lword_frequency_features_prefix'] != 'none':
            ...
            if (len(all_linkwords) > 0):
                ...
                set_linkw_in_textw = set_all_linkwords.intersection(
                    set_all_textwords)
                ...
                fl_textwords = [
                    w for w in all_linkwords if w in list(set_fl_textwords)]
                flword_count = len(fl_textwords)
                print(
                    len(fwl), prm_wf['lword_frequency_features_prefix'], ';', end='')
        ...
        if prm_wf['hword_frequency_features_prefix'] != 'none':
            ...
            all_htmltagwords = all_htmltagwords[:
                                                prm_wc['max_document_html_tag_words']]
            # feature link word count
            if (len(all_htmltagwords) > 0):
                ...
                fh_textwords = [
                    w.strip() for w in all_htmltagwords if w in list(set_fh_textwords)]
                fhword_count = len(fh_textwords)
                print(
                    len(fwh), prm_wf['hword_frequency_features_prefix'], ';', end='')
...
def tfidffeaturemake(soup, word_vectorizer, lword_vectorizer, hword_vectorizer, current_url, cnames, prm_wc, prm_wf, nlp=None):
    ...
    if len(all_textwords) > 0:
        ...
        wordset = wordset.difference(
            set(word_vectorizer.get_feature_names()))  # subtract feature words
        for word in wordset:
            if word in current_url:
                urlword_count += all_textwords.count(word)
        current_features.append(urlword_count)
        ...
        tf_idf_vector = word_vectorizer.transform([" ".join(f_textwords)])[
            0].todense().tolist()[0]
        ...
        if prm_wf['word_frequency_features_prefix'] != 'none':
            print(len(word_vectorizer.get_feature_names()),
                  prm_wf['word_frequency_features_prefix'], ';', end='')
        ...
        if prm_wf['word_positions_stdev_features_prefix'] != 'none':
            print(len(word_vectorizer.get_feature_names()),
                  prm_wf['word_positions_stdev_features_prefix'], ';', end='')
            for feature in fwt.index:
                if fword_count > 0 and all_textwords.count(feature) > 0:
                    current_features.append(stdev(
                        [index for index, w in enumerate(all_textwords) if w == feature])/tword_count)
        ...
        if prm_wf['lword_frequency_features_prefix'] != 'none':
            ...
            if (len(all_linkwords) > 0):
                ...
                set_fl_textwords = set_all_linkwords.intersection(set_fwl)
                fl_textwords = [w.strip()
                                for w in all_linkwords if w in list(set_fl_textwords)]

                ltf_idf_vector = word_vectorizer.transform([" ".join(fl_textwords)])[
                    0].todense().tolist()[0]
                flword_count = len(fl_textwords)
                print(len(lword_vectorizer.get_feature_names()),
                      prm_wf['lword_frequency_features_prefix'], ';', end='')
                ...
                current_features.extend(
                    [0] * len(lword_vectorizer.get_feature_names()))
        # htmltag word frequencies
        if prm_wf['hword_frequency_features_prefix'] != 'none':
            # determine linkwords from soup
            all_htmltagwords = gethtmltagwords(soup, prm_wc)
            ...
            if (len(all_htmltagwords) > 0):
                ...
                fh_textwords = [
                    w.strip() for w in all_htmltagwords if w in list(set_fh_textwords)]
                htf_idf_vector = word_vectorizer.transform([" ".join(fh_textwords)])[
                    0].todense().tolist()[0]
                fhword_count = len(fh_textwords)
                print(len(hword_vectorizer.get_feature_names()),
                      prm_wf['hword_frequency_features_prefix'], ';', end='')
                for i, feature in enumerate(hword_vectorizer.get_feature_names()):
                    current_features.append(htf_idf_vector[i])
            else:
                current_features.extend(
                    [0] * len(hword_vectorizer.get_feature_names()))
...
def featureprep(df, prm):
    ...
    if "ascii_total" in df.columns.values:
        droprow = [i for i in df.index if df.iloc[i, df.columns.get_loc(
            'ascii_total')] < prm['min_ascii_count']]
        print('\nDropping ', len(droprow),
              ' samples with ascii length less than ', prm['min_ascii_count'])
    ...
    print('Dataset has ', X.shape[1],
          ' features and ', X.shape[0], ' tagged samples.')
...
def samplebalance(X, y, prm):
    ...
    for l in yvaluecounts.index:
        ...
        if maxvaluecount > len(yselect):
            yboost = yboost.append(yselect.sample(
                n=maxvaluecount-len(yselect), replace=True, random_state=prm['seed']))
            Xboost = Xboost.append(Xselect.sample(
                n=maxvaluecount-len(yselect), replace=True, random_state=prm['seed']))
